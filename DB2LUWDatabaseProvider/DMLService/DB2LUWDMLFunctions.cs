...
{
    internal class DB2LUWDMLFunctions : BaseDMLFunctions
    {
        ...
        internal DB2LUWDMLFunctions(IDMLService dmlService) : base(dmlService) { }
        ...
        public override string DecimalToText(string d)
        {
			...
			return string.Format("case when {0} = 0 then '0' else TRIM(TRAILING '.' FROM TRIM(TRAILING '0' FROM cast({0} as decimal(31, " + Constants.DecimalDecimals + ")))) end", d);
		}		
		
		public override string IdentifierToInteger(string id)
        {
            return string.Format("COALESCE({0}, 0)", id);
        }
        ...
        {
            return string.Format("COALESCE({0}, '')", id);
        }
		...
		public override string TextToDateTime(string t, string dateFormat)
        {
            ...
            return String.Format("timestamp (substr({0}, 1, 10), substr({1}, 12))", tWithDateReplace, t);
        }
